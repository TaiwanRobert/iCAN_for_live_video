...
def im_detect(sess, net, image_file, Test_RCNN, prior_mask, Action_dic_inv, object_thres, human_thres, prior_flag, detection):

    im_orig, im_shape = get_blob(image_file)
    
    blobs = {}
    blobs['H_num'] = 1
    for Human_out in Test_RCNN:
...
#             dic['image_id']   = image_id
            ...
            # hico
            # Score_obj     = np.empty((0, 5 + 600), dtype=np.float32)

            for Object in Test_RCNN:
                if (np.max(Object[5]) > object_thres) and not (np.all(Object[2] == Human_out[2])): # This is a valid object
                    ...
                    if prior_flag == 3:
                        ...
                        prediction_HO  = prediction_HO * prior_mask[:,Object[4]].reshape(1,29)
                        # hico
                        # prediction_HO  = prediction_HO * prior_mask[:,Object[4]].reshape(1,600)
            ...
            for i in range(29):
                ...
                # hico
                # dic[agent_name] = np.max(Human_out[5])
            ...
            for i in range(29):
                ...
                if (i == 3) or (i == 17) or (i == 22) or (i == 27):
                    ...
                    # hico
                    # dic[Action_dic_inv[i]] = np.append(np.full(4, np.nan).reshape(1,4), np.max(Human_out[5])) 
